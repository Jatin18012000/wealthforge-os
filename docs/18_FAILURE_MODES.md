...
| No confirmed buy/SIP/sell Activity ever recorded (v1.1 IM-03) | Growth Decomposition, Contribution vs Return, Portfolio Performance's XIRR, and Investment Plan Adherence report `insufficient-data` (or, for Plan Adherence, insufficient per month) rather than a fabricated zero contribution or a false "under-invested" reading — the documented snapshot-import path never creates these records from an observed quantity change, so a snapshot-only data set is expected to hit this path |
| Tracked index has no dated `Valuation` at or before a comparison date (v1.1 IM-03, Portfolio vs Benchmark) | That index reports `insufficient-data` for the comparison; never compares against a stale or absent price, and never fetches live data itself — a market refresh (M10) or manual entry populates the same `Valuation` table this widget reads |
