...
| Conflict                                                                                                                | Resolution                                                                                                                                                                      |
| ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Specification implied a simple `Category/Label/Amount` budget table; the real workbook is a positional two-block layout | Reference file wins on _structure_ (Level 4 informs implementation detail); the specification still governs _requirements_. Generic parser retained as a fallback so both work. |
...
observed-change path exactly as designed. What M5 lacked was a _source
adapter for the real layout_, which has been added alongside the existing
...
budget/portfolio _ingestion layouts_, not market-data sourcing. D-007
...
## 9 · M11 re-check (AI Analyst)

Re-scanned all reference material before starting M11: none contains an
AI prompt design, grounding methodology, or provider selection — this was
expected, since `docs/12_AI_ANALYST_SPEC.md` (Level 2/3) and
`docs/decisions/0004-ai-provider-abstraction.md` already fully specify the
AI layer's guardrails and provider default. M11 implements that existing
specification; no reference report informed or could have overridden it.

## 10 · Final verdict (M12)

Across M6 through M11, no milestone after the initial reference pass
required returning to the five reference reports for new schema or
mapping decisions — each later milestone's scope (dashboard, analytics,
manual controls, Data Center, market data, AI) was governed by the
controlling documents (Level 2/3) and this project's own prior decisions,
with the reference reports' contribution fully captured in sections 1–7
above. Reference coverage is complete and stable as of M12.
