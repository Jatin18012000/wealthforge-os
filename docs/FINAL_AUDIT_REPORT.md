...
| D-006 (brokerage integration)       | D/E — requires a user decision   | Raised as the one required question (Kite Connect's ~₹500/month subscription and OAuth-style login vs. Groww's lack of any official API). **User confirmed: leave deferred.** Manual CSV/XLSX import remains the only portfolio data path.                                                                                                                                                                             |
