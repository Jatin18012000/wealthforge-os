...
| IM-06 Scenario Engine | COMPLETE — `src/domain/scenarios.ts` (`projectFutureValue`, `monthsUntilTarget`, `simulateDebtPrepayment` — genuinely new, deterministic domain math since nothing else here projects a future value or amortizes a loan) + `src/views/scenarioEngineView.ts` (SIP Increase Simulator, Debt Prepayment Simulator, Wealth Projection, Financial Independence Projection; Goal Trade-Off Simulator cross-referenced to IM-04, not duplicated); every growth-rate assumption is an observed `computeCagr` since inception, never an invented market-return figure; FI Projection discloses the 4%-rule/25x-expense convention as an explicit external assumption, not a project rule; illustrative comparison points (SIP increase %, prepayment amounts) are clearly labeled as such, not recommendations; wired into the Command Center under "Scenario engine"; 13 domain unit tests + 5 view-model unit tests (each run twice) + 2 E2E tests (each run twice); 450 unit tests and 132 E2E tests passing repo-wide |
| IM-07 AI Intelligence (Daily Brief) | COMPLETE — `src/views/dailyBriefView.ts` builds a `Report` (M10's existing FACT/INFERENCE/RECOMMENDATION structure) from IM-02–IM-05's `Insight<T>` outputs (Position/Changes/Why/Deviations/Risks/Goals/Portfolio/Data quality, 8 sections); fed through the unchanged `explainReport`/`checkGrounding` pipeline from M11 — no changes to `src/ai/*`; surfaced as a second "Generate daily brief" action on the AI Analyst screen sharing one helper with the existing action; full spec in `docs/24_DAILY_BRIEF_SPEC.md`; 2 view-model unit tests + 2 E2E tests (each run twice); 452 unit tests and 134 E2E tests passing repo-wide |
| IM-08 Command Center 2.0 | COMPLETE — `src/app/page.tsx` reorganized into the exact directive order (Daily Brief → tiles → Net Worth Trajectory/Money Flow → Portfolio X-Ray/Risk → Plan vs Reality/Adherence → Goal Radar/EMI Freedom → Wealth Waterfall/Financial Health → What Needs Attention/Data Health → More intelligence → Scenario engine); no view model changed, no widget removed, only reordered; adds a new "Plan vs reality" card exposing the already-computed `comparePlanVsActual` for the first time on the Command Center; Daily Brief surfaced at the top via a new `explainDailyBriefFromHomeAction`; full spec in `docs/25_COMMAND_CENTER_V2_SPEC.md`; 2 new E2E tests plus a widget-regression test (each run twice), replacing 4 retired per-milestone section tests whose target headings no longer exist under the new structure; 452 unit tests and 132 E2E tests passing repo-wide — **all eight v1.1 milestones (IM-01–IM-08) are now complete** |
